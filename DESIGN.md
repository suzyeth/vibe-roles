...
> Implementation details (state machine, schemas, AI calls, UI, fallback, demo data): see SPEC.md — that is the canonical engineering reference; if anything here disagrees with SPEC.md, SPEC.md wins.
...
8. **Ending & result card**: when a player taps **Wrap it up** (or the beat cap is hit), the AI writes a twist ending and a shareable Quest Card.
9. **Return**: the result card footer says **Start your own Vibe Dice on Zymix**.

### 3.2 Recommended length
- Per run: ~3 minutes (variable; players can end anytime).
- Length: turn-based **beats**, not a fixed round count. Beat cap **12**; any player can tap **Wrap it up** after the first 3 beats to jump to the climax/ending.
- Per beat: one player's action (tap an option or free-type) + one roll.
- Friend interference: each friend submits at most 1 Fate Card, to keep the story from spiraling.

> Canonical themes, dice tiers, state machine, and the full demo chain are defined in SPEC.md (§7, §4, §1, §10).
...
- **Tone**: playful, Gen Z, English; PG-safe (no violence/explicit/hate). Keep momentum (short beats), but prioritize vivid, connected story over terseness.
...
8. **Loop & raise stakes.** Repeat turns, escalating each time. Players can trigger **"Wrap it up"** anytime to jump to the CLIMAX (all Final Rolls, then ENDING). External friends may add Fate Cards that the GM weaves into the next beat.
9. **CLIMAX & ending.** When "Wrap it up" triggers or a beat cap is hit, each player takes one Final Roll (no new ACTION_SELECT). The GM writes a twist ending.
...
> **Note:** The demo uses a simplified 3-round fixed loop for time-boxing; the full product supports "Wrap it up" + variable length (see SPEC §1.1).
...
7. **Opt-in & exit anytime.** Starting a quest is opt-in and never @-pings everyone; any player can leave the quest without leaving the chat, and anyone can tap "wrap it up" to jump to the ending. Length is capped by the beat cap (12) OR a time box, whichever comes first.
...
> Dice mapping and labels: see SPEC.md §2.2.
...
| Dice result | Emoji | Color | Story effect |
|---|---|---|---|
| total-chaos (1) | 💀 | #EF4444 | Critical fail, triggers the most absurd consequence |
| awkward-fail (2–5) | 😬 | #F97316 | Failure, but with a funny twist |
| messy-progress (6–10) | 😅 | #EAB308 | Partial success with a cost |
| works-somehow (11–15) | 😌 | #22C55E | Success, but leaves a problem |
| main-character-moment (16–19) | 😎 | #3B82F6 | Success, and you gain an edge |
| iconic-roll (20) | 🔥 | #8B5CF6 | Huge success, a highlight moment |
...
| Story too thin or too long | Cap beats (12) + "Wrap it up", but each beat is 2–4 vivid sentences (§3.3) |
