...
4. **Turn order.** Players act in turn (round-robin by join order). The GM announces whose turn it is.
...
7. **Sync to the group.** EVERY beat — the opening, the cast, each player's chosen action, their roll + result label, and the narration — is posted as messages into the group chat, so everyone watches the story unfold in real time. Other members chime in with in-character reactions each round.
